...
from . import voc2007, flickr, caltech101, imagenetv2, objectnet, caltech101vic, pascal3d, oodcv, occluded_pascal
...
def build_dataset(dataset_name, root="root", transform=None, split="test", download=True, annotation_file=None, language="en", task='zeroshot_classification', cupl=False, wds_cache_dir=None,
    ...
    if dataset_name == "cifar10":
        ...
        ds = pascal3d.Pascal3d(root=root, target_type="category", transform=transform, download=False,
                               split=split, **kwargs)
    elif dataset_name == 'pascal3dCorr':
        corruption = {'corruption': 'all', 'severity': 5}
        ds = pascal3d.Pascal3d(root=root, target_type="category", transform=transform, download=False,
                               split=split, corruption=corruption, **kwargs)
    elif dataset_name == "oodcv":
        ds = oodcv.OODCV(root=root, target_type="category", transform=transform, download=False,
                               split=split, **kwargs)
    elif dataset_name == "occludedPascal":
        ds = occluded_pascal.OccludedPascal(root=root, target_type="category", transform=transform, download=False,
                               split=split, **kwargs)
    else:
