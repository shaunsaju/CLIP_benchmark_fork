...
ARCHIVE_META = {
    ...
    "cropped_val": ("cropped_val.tar", "2fca8e9b66f5916c462dfb56591991e4")
}
...
class ImageNetCropped(ImageFolder):
    ...
    def parse_archives(self) -> None:
        ...
        if not os.path.isdir(self.split_folder):
            if self.split == "train":
                parse_cropped_train_archive(self.root)
            elif self.split == "val":
                parse_cropped_val_archive(self.root)

    @property
    def split_folder(self) -> str:
        return os.path.join(self.root, f"cropped_{self.split}")
...
def parse_cropped_train_archive(root: str, file: Optional[str] = None, folder: str = "cropped_train") -> None:
    ...
    Args:
        ...
        folder (str, optional): Optional name for train images folder. Defaults to
            'train'
    """
    raise NotImplementedError(f"Train images not cropped yet")
...
def parse_cropped_val_archive(
        root: str, file: Optional[str] = None, wnids: Optional[List[str]] = None,
        folder: str = "cropped_val"
) -> None:
    ...
    Args:
        ...
        folder (str, optional): Optional name for validation images folder. Defaults to
            'val'
    """
    archive_meta = ARCHIVE_META["cropped_val"]
    if file is None:
        file = archive_meta[0]
    md5 = archive_meta[1]
    if wnids is None:
    ...
    val_root = root # os.path.join(root, folder)  # tar already contains subfolder
    extract_archive(os.path.join(root, file), val_root)

    # # not needed, since we already have the images in the correct folders
